# Edit course function needs to be updated there will be bug otherway.
...
def select_all_semester():
    db_cursor.execute("""SELECT name FROM semester""")
...
def find_section_rowid(name, classroom, hour, day):
...
def edit_section(ID, name, classroom, hour, day):
    if name is not None:
...
def find_announcement_rowid(date, head, announcement):
...
def edit_announcement(ID, date, head, announcement):
    if date is not None:
...
def find_subject_rowid(sub, ref, week):
...
def edit_subject(ID, sub, ref, week):
    if sub is not None:
...
def insert_evaluation(name="Empty", percentage=0):
...
def find_evaluation_rowid(name, percentage):
...
def edit_evaluation(ID, name, percentage):
    if name is not None:
...
def find_note_rowid(head, note, date):
...
def edit_note(ID, head, note, date):
    if head is not None:
...
def insert_student_evaluationin(semID, cID, evaID, stuID, evaluation):
    if (evaluation >= 0) and (evaluation <= 100):
